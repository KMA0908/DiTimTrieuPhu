...
import com.nhn.ditimtrieuphu.common.GameConstant;
...
import com.nhn.ditimtrieuphu.view.dialog.BarChartQuestion;
import com.nhn.ditimtrieuphu.view.dialog.ConfirmStopDialog;
import com.nhn.ditimtrieuphu.view.dialog.DialogProfileUser;
import com.nhn.ditimtrieuphu.view.dialog.GameResultDialog;
import com.nhn.ditimtrieuphu.view.dialog.HelpCallDialog;
import com.nhn.ditimtrieuphu.viewmodel.MainFragViewModel;
import java.util.ArrayList;
import java.util.List;
import java.util.Random;

public class M003PlayFragment extends BaseFragment<MainFragViewModel> implements ConfirmStopDialog.OnCallBack {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.icon_person :
                DialogProfileUser dialogProfileUser = new DialogProfileUser();
                dialogProfileUser.show(getParentFragmentManager(), DialogProfileUser.TAG);
                break;
            case R.id.iv_caseA :
            ...
            case R.id.iv_stop:
                ...
                dialog.setDataResult(tvMoney.getText().toString(), index, true);
    ...
    private void showCallDialog() {
        if(mPlayer == null){
            mPlayer = MediaPlayer.create(App.getInstance(), R.raw.help_call);
        }
        mPlayer.start();
        HelpCallDialog helpCallDialog = new HelpCallDialog();
        helpCallDialog.show(getParentFragmentManager(), HelpCallDialog.TAG);
    ...
    private void showAudienceHelp() {
        if(mPlayer == null){
            mPlayer = MediaPlayer.create(App.getInstance(), R.raw.khan_gia);
        }
        mPlayer.start();
        mPlayer.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
            @Override
            public void onCompletion(MediaPlayer mp) {
                mPlayer.release();
                mPlayer = null;
                mp = MediaPlayer.create(App.getInstance(), R.raw.hoi_y_kien_chuyen_gia_01b);
                mp.start();
                mp.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
                    ...
                    public void onCompletion(MediaPlayer mp) {
                        ...
                        barChartQuestion.show(getParentFragmentManager(),BarChartQuestion.TAG);
                        // Minh: thuc hien xong su tro giup resume timer
                        countDownQuestion(mSecondsLeft);
                    }
                });
    ...
    private void playHelp50Music() {
        if(mPlayer == null){
            mPlayer = MediaPlayer.create(App.getInstance(), R.raw.sound5050);
        }
        mPlayer.start();
        mPlayer.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
            @Override
    ...
    private void handleUserAnswer(int id) {
        ...
        if(mPlayer == null){
            mPlayer = MediaPlayer.create(App.getInstance(), idSource);
        } else {
        ...
        mPlayer.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
            ...
            public void onCompletion(MediaPlayer mp) {
                mPlayer.release();
                mPlayer = null;
                mp = MediaPlayer.create(App.getInstance(), finalTrueAns);
                mp.start();
                mp.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
                    @Override
                    public void onCompletion(MediaPlayer mp) {
    ...
    private void finishGame() {
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                dialog.setDataResult(money, index, false);
                dialog.setmConfirmExecutable(objects -> {
                ...
                dialog.show(getChildFragmentManager(), GameResultDialog.TAG);
            }
        },2500);
    }
    ...
    private void showAlertDialogWinner() {
        ...
        App.getInstance().getStorage().resetPlaySession();
        if(mPlayer == null){
            mPlayer = MediaPlayer.create(App.getInstance(), R.raw.best_player);
        }
        mPlayer.start();
        mPlayer.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
            @Override
            public void onCompletion(MediaPlayer mp) {
                ...
                mPlayer = null;
                // Tạo AlertDialog
                AlertDialog alertDialog = new AlertDialog.Builder(App.getInstance())
                        .setTitle("Chúc mừng")
